...
class CanCan {
	...
	can(performer, action, target, options) {
		return this.abilities.filter(ability => this.instanceOf(performer, ability.model)).filter(ability => {
			return ability.target === 'all' || target === ability.target || this.instanceOf(target, ability.target);
		}).filter(ability => {
